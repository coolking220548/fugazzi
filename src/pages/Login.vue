<template>
  <div class="login-container">
    ...
    <div class="login-links">
      ...
      <router-link to="/register" class="login-link">Create account</router-link>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async handleLogin() {
        ...
        let emailToUse = this.username;
        if (!this.isEmail(this.username)) {
            const email = await this.getEmailByUsername(this.username);
            if (!email) {
                this.error = "Username not found. Please check and try again.";
                return;
            }
            emailToUse = email;
        }
...
.input-group {
  position: relative;
  margin-bottom: 18px;
  width: 100%; /* ensure labels and input take full container width */
}
...
.login-container input {
  width: 100%;
  padding: 10px 12px 10px 36px; /* leave space for icon */
