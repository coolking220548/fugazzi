    ...
    <form @submit.prevent="handleNext">
      ...
      <div v-if="step === 1">
        ...
        <div class="input-group">
          <FontAwesomeIcon :icon="byPrefixAndName.fas['envelope']" class="input-icon" />
          <input v-model="email" type="email" placeholder="Email" required />
        </div>

        <div class="input-group">
          <FontAwesomeIcon :icon="byPrefixAndName.fas['lock']" class="input-icon" />
      ...
      <div v-else>
        <div class="input-group">
          <FontAwesomeIcon :icon="byPrefixAndName.fas['user']" class="input-icon" />
          <input v-model="username" type="text" placeholder="Username" required />
        </div>

        <button type="submit" class="primary-btn">Finish Registration</button>
      </div>
    </form>
  ...
  methods: {
    async registerWithGoogle() {
      ...
      try {
        ...
        // If displayName exists, skip username step
    ...
    async handleNext() {
      ...
      if (this.step === 1) {
        ...
        try {
          ...
          await updateProfile(this.firebaseUser, { displayName: this.username });

          // Save username → email mapping in Firestore
          const usernameDocRef = doc(db, "usernames", this.username);
          await setDoc(usernameDocRef, { email: this.firebaseUser.email });

          // Save auth token & login state
          const idToken = await this.firebaseUser.getIdToken();
          localStorage.setItem("firebaseToken", idToken);
          localStorage.setItem("isLoggedIn", "true");
          window.dispatchEvent(new Event("auth-changed"));
